...
def set_gan(gan_name):
    ...
    try:
        ...
        gan = Gan()
        gan.vocab_size = 5000   # DSD:词汇大小设为5000
        gan.generate_num = 10000    # DSD:生成数量？？
        return gan
    except KeyError:
...
def set_training(gan, training_method):
    try:
        ...
        print(Fore.RED + 'Unsupported training setting: ' + training_method + Fore.RESET)
        sys.exit(-3)
    return gan_func     # DSD:返回一个gan方法的引用
...
def parse_cmd(argv):
    try:
        ...
        if not '-t' in opt_arg.keys():  # 默认训练oracle数据
            gan.train_oracle()
        else:
...
if __name__ == '__main__':
    ...
    '''
    gan = None
    Debug = ['-g', 'rankgan', '-t', 'real', '-d', 'data/emnlp_news.txt']
    parse_cmd(Debug)