...
class OracleLstm(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim, sequence_length, start_token, ):
        ...
        self.emb_dim = emb_dim      # DSD: emb_dim貌似是LSTM的输入参数
        self.hidden_dim = hidden_dim    # DSD: hidden_dim貌似是LSTM的输出参数
        self.sequence_length = sequence_length  # DSD: 句子长度定为20
        self.start_token = tf.constant([start_token] * self.batch_size, dtype=tf.int32)     # DSD:token起始点
        self.g_params = []
    ...
    def create_recurrent_unit(self, params):
        ...
        # DSD:这里Wi和Ui和bi分别表示权重和偏差项，用于控制与输入门相关的权重
        # DSD:这里Wf和Uf和bf分别表示权重和偏差项，用于控制与遗忘门相关的权重
        # DSD:这里Wo和Uo和bo分别表示权重和偏差项，用于控制与输出门相关的权重
        # DSD:这里Wc和Uc和bc分别表示权重和偏差项，用于控制与记忆单元相关的权重
